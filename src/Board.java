 ...
 public boolean move(int r, int c, int spaces) 
  {
   ...
   if (b[r][c] == 1) //The index is the head of the vehicle
   {
	...
	if (v.getClass().getSimpleName().equals("Car"))
	 ...
	 {
	  if (spaces >= b[0].length - c || c + spaces < 0 ) 
	  {
      System.out.println("Your spaces are too high or too low");
      return false;
    }
    if(spaces > 0)
    {
    ...
    else
    {
	 ...
	 {
	  if (spaces >= b.length - r || r - spaces >= b.length) 
	  {
       System.out.println("Your spaces are too high or too low");
       return false;
    }
